...
class DropoutLayer(StochasticLayer):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        if stochastic:
            mask_shape = list(inputs.shape)
            if self.share_across_batch:
                mask_shape[0] = 1
            uniform_sample = self.rng.uniform(0.0, 1.0, size=mask_shape)
            self.mask = (uniform_sample < self.incl_prob).astype(inputs.dtype)

            if self.share_across_batch:
                self.mask = np.repeat(self.mask, inputs.shape[0], axis=0)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        if self.mask is not None:
            ...
            grads_wrt_inputs = grads_wrt_outputs * self.incl_prob
